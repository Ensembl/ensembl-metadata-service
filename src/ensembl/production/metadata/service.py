...
def get_top_level_statistics(metadata_db, organism_id):
    ...
    if len(stats_results) > 0:
        ...
        return create_top_level_statistics(
            {"organism_id": organism_id, "statistics": statistics}
        )
    else:
        return create_top_level_statistics()
...
def get_genomes_from_assembly_accession_iterator(metadata_db, assembly_accession):
    ...
    genome_release = db.Table(
        "genome_release", sqlalchemy_md, autoload_with=metadata_db
    )
    ...
    genome_select = (
        get_genome_query(genome, genome_release, release, assembly, organism)
        .select_from(genome)
        .join(genome_release)
        .join(release)
        .join(assembly)
        .join(organism)
        .where(assembly.c.accession == assembly_accession)
    )
...
def get_sub_species_info(metadata_db, organism_id):
    ...
    # Reflect existing tables, letting sqlalchemy load linked tables where possible.
    organism_group_member = db.Table(
        "organism_group_member", md, autoload_with=metadata_db
    )
    ...
    sub_species_select = (
        ...
        .select_from(organism_group_member)
...
def get_genomes_by_keyword_iterator(metadata_db, keyword, release_version):
    ...
    genome_query = (
        ...
        .where(
            or_(
                func.lower(assembly.c.tol_id) == keyword.lower(),
    ...
    for _, genome_release_group in itertools.groupby(
        genome_results, lambda r: r["assembly_accession"]
    ):
        most_recent_genome = sorted(
            genome_release_group, key=lambda g: g["release_version"], reverse=True
        )[0]
...
def get_genome_by_name(metadata_db, ensembl_name, site_name, release_version):
    ...
    genome_select = (
        ...
        .join(assembly)
        .join(organism)
        .filter_by(ensembl_name=ensembl_name)
    )

    if release_version == 0:
        genome_select = (
            genome_select.join(genome_release).join(release).filter_by(is_current=True)
        )
    else:
        genome_select = (
            genome_select.join(genome_release)
            .join(release)
            .filter_by(version=release_version)
        )
...
def get_datasets_list_by_uuid(metadata_db, genome_uuid, release_version=0):
    ...
    datasets_select = (
        ...
        .join(dataset)
        .join(dataset_type)
        .join(ensembl_release, isouter=True)
        .where(genome.c.genome_uuid == genome_uuid)
    ...
    if len(datasets_results) > 0:
        ...
        return create_datasets(
            {"genome_uuid": genome_uuid, "datasets": dataset_object_dict}
        )
